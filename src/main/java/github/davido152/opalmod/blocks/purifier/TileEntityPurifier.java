...
public class TileEntityPurifier extends TileEntity implements ITickable
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.purifier");
	}
	...
	public static boolean isBurning(TileEntityPurifier te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			BlockPurifier.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = PurifierRecipes.getInstance().getPurifierResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = PurifierRecipes.getInstance().getPurifierResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
